...
import (
	...
	"github.com/SOOA-swarch-2022ii/sooa_user_token_ms_dos/models"
...
func Login(response http.ResponseWriter, request *http.Request) {
	...
	if CheckPasswordHash(login.Password, usuario.Password) {
		...
		//delete previous token of the user
		filtrodelete := bson.D{{"user", login.Username}}
		_, errdelete := TokenDBhandler.DeleteMany(ctx, filtrodelete)
		if errdelete != nil {
			response.WriteHeader(http.StatusInternalServerError)
			response.Write([]byte(`{"error borrando token anterior"}`))
			return
		}
